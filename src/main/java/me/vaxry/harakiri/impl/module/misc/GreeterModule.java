...
public final class GreeterModule extends Module {
    ...
    public void onPlayerJoin(EventPlayerJoin event) {
        switch (this.mode.getValue()) {
            case CLIENT:
                Harakiri.INSTANCE.logChat(event.getName() + " has joined the game");
                break;
            case SERVER:
                Harakiri.INSTANCE.getChatManager().add(event.getName() + " has joined the game");
                break;
        }
    }
    ...
    public void onPlayerLeave(EventPlayerLeave event) {
        switch (this.mode.getValue()) {
            case CLIENT:
                Harakiri.INSTANCE.logChat(event.getName() + " has left the game");
                break;
            case SERVER:
                Harakiri.INSTANCE.getChatManager().add(event.getName() + " has left the game");
                break;
        }
    }
}
