...
public final class ESPModule extends Module {
            ...
            if (e != null) {
                ...
                if (this.checkFilter(e)) {
                    ...
                    if (bounds != null) {
                        ...
                        headx = bounds[0] + (bounds[2] - bounds[0]) / 2;
                        heady = bounds[1] + (bounds[3] - bounds[1]);

                        float scaleXY = 1;

                        if(d0 < 5){
                            GlStateManager.pushMatrix();
                            GlStateManager.scale(5 - d0, 5 - d0, 5 - d0);
                            scaleXY = 5 - (float)d0;
                        }
                        ...
                        if (this.nametag.getValue()) {
                            ...
                            final Friend friend = Harakiri.INSTANCE.getFriendManager().isFriend(e);
                            ...
                            if (this.background.getValue()) {
                                RenderUtil.drawRect(headx - mc.fontRenderer.getStringWidth(name) / 2.0f - 1, heady - mc.fontRenderer.FONT_HEIGHT - 2, headx + mc.fontRenderer.getStringWidth(name) / 2.0f + 1, heady - 1, 0x75101010);
                            }

                            mc.fontRenderer.drawStringWithShadow(name, headx/scaleXY - mc.fontRenderer.getStringWidth(name) / 2.0f, heady/scaleXY - mc.fontRenderer.FONT_HEIGHT - 1, color);
                        }

                        if (e instanceof EntityPlayer) {
                            final EntityPlayer player = (EntityPlayer) e;

                            final Coordinate headPos = conv3Dto2DSpace(e.posX, e.posY+((EntityPlayer) e).eyeHeight, e.posZ);
                            ...
                            if (this.ping.getValue()) {
                                ...
                                if (this.background.getValue()) {
                                    RenderUtil.drawRect((headx - mc.fontRenderer.getStringWidth(name) / 2.0f - 1)/scaleXY, (heady - mc.fontRenderer.FONT_HEIGHT - 2)/scaleXY, (headx + startX + mc.fontRenderer.getStringWidth(pingFormatted))/scaleXY, (heady - 1)/scaleXY, 0x75101010);
                                }

                                mc.fontRenderer.drawStringWithShadow(pingFormatted, (headx + startX)/scaleXY, (heady - mc.fontRenderer.FONT_HEIGHT - 1)/scaleXY, pingColor);
                            }
                        }
                        ...
                        if (e instanceof EntityLivingBase) {
                            ...
                            if (this.hearts.getValue()) {
                                ...
                                if (this.background.getValue()) {
                                    RenderUtil.drawRect(headx + startX, heady - mc.fontRenderer.FONT_HEIGHT - 2, headx + startX + mc.fontRenderer.getStringWidth(heartsFormatted), heady - 1, 0x75101010);
                                }

                                mc.fontRenderer.drawStringWithShadow(heartsFormatted, headx + startX, heady - mc.fontRenderer.FONT_HEIGHT - 1, heartsColor);
                            }
                            ...
                            if (e instanceof EntityTameable || e instanceof AbstractHorse) {
                                if (this.owner.getValue()) {
                                    ...
                                    if (this.background.getValue()) {
                                        RenderUtil.drawRect(headx + startX, heady - mc.fontRenderer.FONT_HEIGHT - 2, headx + startX + mc.fontRenderer.getStringWidth(ownerName), heady - 1, 0x75101010);
                                    }

                                    mc.fontRenderer.drawStringWithShadow(ownerName, headx + startX, heady - mc.fontRenderer.FONT_HEIGHT - 1, 0xFFAAAAAA);
                                }
                            }
                            ...
                            for (ItemStack stack : stacks) {
                                if (stack != null) {
                                    ...
                                    if (item != Items.AIR) {
                                        ...
                                        GlStateManager.translate(headx + x - (16 * stacks.size() / 2.0f), heady - mc.fontRenderer.FONT_HEIGHT - 19, 0);
                                        if (this.background.getValue()) {
                                        ...
                                        if (this.enchants.getValue()) {
                                            ...
                                            for (String string : stringsToDraw) {
                                                GlStateManager.pushMatrix();
                                                GlStateManager.disableDepth();
                                                GlStateManager.translate(headx + x - ((16.0f * stacks.size()) / 2.0f) - (16.0f / 2.0f) - (mc.fontRenderer.getStringWidth(string) / 4.0f), heady - mc.fontRenderer.FONT_HEIGHT - 23 - y, 0);
                                                GlStateManager.scale(0.5f, 0.5f, 0.5f);
                                                if (this.background.getValue()) {
                        ...
                        if(d0 < 5){
                            GlStateManager.scale(1/(5 - d0), 1/(5 - d0), 1/(5 - d0));
                            GlStateManager.popMatrix();
                        }
                    }
                }
    ...
    // RenderLivingBase.java:211
    // use subscribeevent to get this
    // render stuf uwu
    // https://github.com/Hexeption/Opiates/blob/master/Patch/Minecraft.patch line 546


    // just setup mixins

    @SubscribeEvent
    public void onRenderLivingEvent(RenderLivingEvent.Specials.Pre<EntityLivingBase> e) {
        /*Class[] cArg = new Class[7];
        cArg[0] = e.getEntity().getClass();
        for(int i = 1; i < 7; ++i)
            cArg[i] = Float.class;
        Method render = e.getRenderer().getClass().getDeclaredMethod("renderModel", cArg);
        render(e.getEntity(),e.getEntity().limbSwing, e.getEntity().limbSwingAmount, 0, )
