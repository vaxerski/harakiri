...
public final class ScaffoldModule extends Module {
    ...
    public void onMotionUpdate(EventUpdateWalkingPlayer event){
        ...
        BlockPos feetBlock = GetLocalPlayerPosFloored().down();
        ...
        {
            ValidResult result = valid(feetBlock);
            ...
            for (final EnumFacing side : EnumFacing.values())
            {
                ...
                if (mc.world.getBlockState(neighbor).getBlock().canCollideCheck(mc.world.getBlockState(neighbor), false))
                {
                    ...
                    if (eyesPos.distanceTo(hitVec) <= 5.0f)
                    {
                        ...
                        PacketFacePitchAndYaw(rotations[1], rotations[0]);
                        break;
                    }
                }
            }

            if (place(toPlaceAt, 5.0f, false, false, true) == PlaceResult.Placed)
            {
