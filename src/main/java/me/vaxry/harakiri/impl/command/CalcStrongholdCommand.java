    ...
    public void exec(String input) {
        ...
        if (split.length > 1) {
            if (equals(resetAlias, split[1])) {
                Harakiri.INSTANCE.logChat("Reset Stronghold finder");
            ...
            Harakiri.INSTANCE.logChat("Please throw the first Eye Of Ender");
        }
    }
        ...
        if (event.getStage() == EventStageable.EventStage.PRE) {
            if (this.firstStart != null && this.firstEnd != null && this.secondStart != null && this.secondEnd != null) {
                ...
                if (Double.isNaN(intersection[0]) || Double.isNaN(intersection[1]) || Double.isInfinite(intersection[0]) || Double.isInfinite(intersection[1])) {
                    Harakiri.INSTANCE.errorChat("Error lines are parallel");
                    Harakiri.INSTANCE.getEventManager().removeEventListener(this);
                    return;
                }
                ...
                Harakiri.INSTANCE.logcChat(component);
                Harakiri.INSTANCE.getNotificationManager().addNotification("", "Stronghold found " + ChatFormatting.GRAY + (int) dist + "m away");
                this.firstStart = null;
                this.firstEnd = null;
                this.secondStart = null;
                this.secondEnd = null;
            ...
            if (event.getPacket() instanceof SPacketSpawnObject) {
                ...
                if (packet.getType() == 72) {
                    if (this.firstStart == null) {
                        ...
                        Harakiri.INSTANCE.logChat("Found first Eye Of Ender start");
                    } else {
                        if (this.secondStart == null) {
                            this.secondStart = new Vec3d(packet.getX(), packet.getY(), packet.getZ());
                            Harakiri.INSTANCE.logChat("Found second Eye Of Ender start");
                        }
                    }
                }
            }
            if (event.getPacket() instanceof SPacketSoundEffect) {
                ...
                if (packet.getSound() == SoundEvents.ENTITY_ENDEREYE_DEATH) {
                    if (this.firstEnd == null) {
                        ...
                        Harakiri.INSTANCE.logChat("Found first Eye Of Ender end");
                        Harakiri.INSTANCE.logChat("Please throw the second Eye Of Ender");
                    } else {
                        if (this.secondEnd == null) {
                            this.secondEnd = new Vec3d(packet.getX(), packet.getY(), packet.getZ());
                            Harakiri.INSTANCE.logChat("Found second Eye Of Ender end");
                        }
                    }
                }
            }
