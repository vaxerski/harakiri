        ...
        if (item != null) {
            ...
            if (split.length >= 3) {
                if (StringUtil.isInt(split[2])) {
                    ...
                    Harakiri.INSTANCE.errorChat("Unknown number " + "\247f\"" + split[2] + "\"");
                }
            }
            ...
            Harakiri.INSTANCE.logChat("Gave you " + amount + " " + itemStack.getDisplayName());
        } else {
            final ResourceLocation similar = this.findSimilarItem(split[1]);

            if (similar != null) {
                Harakiri.INSTANCE.errorChat("Unknown item " + "\247f\"" + split[1] + "\"");
                Harakiri.INSTANCE.logChat("Did you mean " + "\247c" + similar.getPath() + "\247f?");
            }
        }
    }
