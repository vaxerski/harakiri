...
public final class ModuleCommand extends Command {
    ...
    public void exec(String input) {
        ...
        final int size = Harakiri.INSTANCE.getModuleManager().getModuleList().size();

        final TextComponentString msg = new TextComponentString("\2477Modules [" + size + "]\247f ");

        final CommandsModule commandsModule = (CommandsModule) Harakiri.INSTANCE.getModuleManager().find(CommandsModule.class);

        for (int i = 0; i < size; i++) {
            final Module mod = Harakiri.INSTANCE.getModuleManager().getModuleList().get(i);
            if (mod != null) {
