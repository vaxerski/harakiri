...
void main(){
	...
	vec3 outColor;
	if(islow(center.rgb) || islow(left.rgb) || islow(right.rgb) || islow(up.rgb) || islow(down.rgb)) {
		outColor = center.rgb * 0 + left.rgb * 0 + right.rgb * 0 + up.rgb * 0 + down.rgb * 0;
	} else {
		outColor = center.rgb * center.a + left.rgb * left.a + right.rgb * right.a + up.rgb * up.a + down.rgb * down.a;
	}
